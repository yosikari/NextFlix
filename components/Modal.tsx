    ...
    const handleList = async () => {
        if (isAddedToList) {
            await deleteDoc(
                ...
                console.log(error)
                toast(`Failed to remove ${movie?.title || movie?.original_name}`,
                    {
                        duration: 5000,
                        style: toastStyle
                    })
            ...
            await setDoc(
                doc(db, 'customers', user!.uid, 'myList', movie?.id.toString()!),
                {
                    ...
                    console.log(error)
                    toast(`Failed to add ${movie?.title || movie?.original_name}`,
                        {
                            duration: 5000,
                            style: toastStyle
                        })
    ...
    >
        <>
            ...
            <button onClick={handleClose}
                className="modalBtn absolute right-5 top-5 !z-40 h-9 w-9 border-none bg-{#181818] hover:bs-[#181818]">
            ...
            <div className="relative pt-[56.25%]">
                ...
                <div className='absolute bottom-10 flex w-full justify-between px-10'>
                    ...
                    <button
                        ...
                        {muted ? <FaVolumeMute className="h-7 w-7" />
                ...
                <div className="space-y-6 text-lg">
                    ...
                    <div className='flex flex-col gap-x-10 gap-y-4 font-light md:flex-row'>
                        ...
                        <div className='flex flex-col space-y-3 text-sm'>
                            ...
                            <div>
                                <span className="text-[gray] mr-1">Language:</span>{' '}
                                {movie?.original_language.toUpperCase()}
                            </div>
                            <div>
                                <span className="text-[gray] mr-1">Total votes:</span>{' '}
                                {movie?.vote_count}
                            </div>
                        </div>
                    </div>
